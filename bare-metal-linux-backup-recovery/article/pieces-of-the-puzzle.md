...
An even worse case is operating system failure. Some errors may still be recoverable, but an unbootable or badly damaged operating system usually means rebuilding the machine from scratch unless a proper recovery procedure has already been put in place. This could be caused by filesystem corruption, a failed upgrade that leaves the bootloader or kernel unusable, serious storage failure, or a destructive administrative mistake such as overwriting the wrong filesystem.

There is also another case where the operating system may still appear to work perfectly well but hides a more sinister failure: a server that has been compromised. Once an attacker has gained sufficient access, repairing the visible damage does not necessarily mean the system can be trusted again — backdoors that are difficult to find may have been left behind — and rebuilding from a known-good state may be the only safe option.
