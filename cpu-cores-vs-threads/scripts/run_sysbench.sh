...
#
# Runs the sysbench CPU benchmark with 1-12 worker threads and
...
MASTER="$BASE/sysbench-master-${RUNSTAMP}.txt"
...
run_test()
{
    ...
    local prefix
    prefix="sysbench-t$(printf '%02d' "$threads")-r${pass}"
    ...
    # Resume support: a completion marker means this configuration
    # already passed benchmark execution and post-test validation.
    if [[ -f "$done" ]]
    then
    ...
    # Preserve any result whose validation did not complete successfully.
    ...
    then
        ...
        echo "Pass: $pass"
        echo
        echo "Run run_sysbench.sh again later to resume here."
        exit 1
    fi
    ...
    then
        ...
        archive_incomplete_result "$result"
        echo "Run run_sysbench.sh again later to retry this configuration."
        exit 1
    fi
    ...
    then
        ...
        echo
        echo "Restart run_sysbench.sh later."
...
echo "=================================================="
echo "SYSBENCH CPU SCALING BENCHMARK"
...
echo
echo "=================================================="
echo "ALL SYSBENCH RUNS COMPLETED"
echo "Completed: $(date --iso-8601=seconds)"
echo "=================================================="
